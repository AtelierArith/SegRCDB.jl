...
# ╔═╡ f4208844-cbb7-422e-b11c-b2a1813de1c8
begin
...
# ╔═╡ ef546406-5f84-11ee-2277-ed07191e530c
md"""
# Study SegRCDB_render.jl
"""

# ╔═╡ ca6a4918-de57-461f-8744-e15e978484f9
md"""

Based on the following parameters described in `make_SegRCDB.sh` not (`SegRCDB_render.py`) we would like to define structs(/types) named `SegRCDBRuntimeConfig` and `SegRCDBDatasetConfig` which are something like classes in terms of Python syntax.
...
# ╔═╡ 7615013f-a39b-4cea-8501-a742039c64f1
...
# ╔═╡ 31cb67f5-b7c5-49ad-8df9-7cbae30d07de
...
# ╔═╡ 1f12da1c-29ad-4102-8e79-e4e47ba2a568
md"""
```python
# SegRCDB_params.py
# csv file save
...
```console
$ cat SegRCDB-dataset/param/00001.csv
Category_num,1
Vertex,186
Perlin_noise,3.560975683348524
line_width,0.004048437818077755
Center_rad,32
Line_num,49
Oval_rate_x,1.4859276965628125
Oval_rate_y,1.9182343317851318
Color_Gray,1
```
...
# ╔═╡ b32109bd-1b7d-4f77-885c-87bc984bce98
...
# ╔═╡ 5a95961e-9917-4f2f-9de8-b4b6ca1f6829
begin
    function uniform(
...
# ╔═╡ 552b6f5d-bc5e-4f21-8129-9f86681db48e
begin
    #=
...
# ╔═╡ 525e8b30-375f-4b2c-8250-c37a709a6b4c
begin
    mkpath(joinpath(save_root, "image"))
    mkpath(joinpath(save_root, "mask"))
    param_dir_root = joinpath(save_root, "param")
end

# ╔═╡ 9d22f263-76f1-436d-bca2-099686386504
begin
    rng = Xoshiro(1234)
    canvas = zeros(RGB{N0f8}, image_size, image_size)
    mask = zeros(Gray{N0f8}, image_size, image_size)

    #for image_id in 1:numof_images
    for image_id in 1:instance_num
        ...
        for line_draw in 1:line_draw_num
            for i in eachindex(noise_x, noise_y)
                ...
                nx = pnoise1(xu) * perlin_noise_coefficient * 2 - perlin_noise_coefficient
                noise_x[i] = nx

                yu = uniform(rng, 0, 10000)
                ny = pnoise1(yu) * perlin_noise_coefficient * 2 - perlin_noise_coefficient
                noise_y[i] = ny
            end
            ...
            fname = @sprintf "/mask/%06d.png" image_id
            #mask_save.save(joinpath(args.save_root, fname), quality = 95)
...
# ╔═╡ 3a4a630a-abcc-48be-a9f0-810250199a08
canvas

# ╔═╡ 2102e66b-7d75-4e8e-83c0-7bb9bd676462
mask
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "38e06b04d8cf5124e9b105f7cd5122e62ee8631e"
